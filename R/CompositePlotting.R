...
CompositePlot = function(CompositeDF, span=0.1, TF){
  ...
  CompositeDF %>%
    ...
    {if(!PlotHex){geom_point(aes(color = Sample), alpha=0.5)}} +
    # {if(PlotHex){geom_hex()}} +
    {if(PlotHex){ggpointdensity::geom_pointdensity(adjust = 0.1, size = .5)}} +
    ...
    facet_wrap(~Sample) +
    {if(PlotHex){viridis::scale_color_viridis(trans = "log10")}} +
    theme_classic() 
